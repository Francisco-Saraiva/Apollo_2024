...
// print_vector is overloaded in order to be able to use multiple data types with it
void print_vector(float* vector, int vector_length){
  //int f = 0;
...
void loop() {
  if (Serial.available() > 0){
    ...
    if (counter == BUFFER_SIZE){
      ...
      delay(5000);
      //print the strings 
      print_vector(signalBuffer, BUFFER_SIZE);
      delay(2000);
      print_vector(signalLow, new_size);
      delay(2000);
      print_vector(signalNotch, new_size);
      delay(2000);
      print_vector(frequencies, fft_sz);
      delay(2000);
      print_vector(freq_amps, fft_sz);
      delay(2000);
      print_vector(beta_freqs, 2);
      delay(2000);
      print_vector(beta_freq_amps, beta_size);
